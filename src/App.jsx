...
const DEFAULTS = {
  ...
  myRetireAge: 65, myPension: 150, myCash: 200, myNisaBalance: 0, myRetireBonus: 500,
  hasSpouse: true,
  spouseAge: 28, spouseTakeHome: 250, spouseIncomeGrowth: 1.0,
  spouseRetireAge: 63, spousePension: 100, spouseCash: 100, spouseNisaBalance: 0, spouseRetireBonus: 0,
  ...
  houses: [],
  propertyInitialValue: 0,
  propertyBuildYear: BASE_YEAR,
  propertyDepreciationRate: 1.5,
  propertyTaxEnabled: true,
  propertyTaxRate: 0.14,
  // 生活費
  ...
  cashReturn: 0.3,
  investPeriod1End: BASE_YEAR+4,
  investPeriod2End: BASE_YEAR+9,
...
// ── 変動係数 ──────────────────────────────────────────────────────
const INCOME_COEF = [
  { max: 19, c: 1.321 }, { max: 24, c: 1.046 }, { max: 29, c: 1.024 },
  { max: 34, c: 1.024 }, { max: 39, c: 1.017 }, { max: 44, c: 1.011 },
  { max: 49, c: 1.011 }, { max: 54, c: 1.005 }, { max: 59, c: 1.005 },
  { max: 64, c: 1.005 }, { max: 69, c: 1.005 }, { max: 999, c: 0.883 },
];
...
function simulate(p) {
  ...
  let myNisa = p.myNisaBalance || 0;
  let spouseNisa = p.spouseNisaBalance || 0;
  ...
  for (let age = p.myAge; age <= p.lifeExpectancy; age++) {
    ...
    const spouseInc = !p.hasSpouse ? 0
      : spouseRetired ? p.spousePension
      : inAdj ? spouseRaw * (p.spouseAdjRatio/100)
      : spouseRaw;
    ...
    houses.forEach(h => {
      ...
      if (h.loan > 0 && yr >= h.year && yr < h.year + h.loanYears) {
        const lm = (h.loan * (h.loanRate/100/12) * Math.pow(1+h.loanRate/100/12, h.loanYears*12)) /
                   (Math.pow(1+h.loanRate/100/12, h.loanYears*12) - 1);
        loanCost += lm * 12;
      }
    });
    ...
    const basePrice = houses.length > 0 ? (houses[0].cashPrice + houses[0].loan)
                    : (p.propertyInitialValue || 0);
    ...
    const g = (k) => p[k + iP] || 0;
    const myNisaC = !bothRetired && cy >= p.myNisaStart && cy <= p.myNisaEnd ? g('myNisaAnnual') : 0;
    const spouseNisaC = p.hasSpouse && !bothRetired && cy >= p.spouseNisaStart && cy <= p.spouseNisaEnd ? g('spouseNisaAnnual') : 0;
    ...
    houses.forEach(h => {
      if (h.investWithdraw > 0 && yr === h.year) {
        ...
        if (h.investSource === "nisa") {
          const av = myNisa + spouseNisa; const tk = Math.min(w, av); const r = av > 0 ? tk/av : 0;
          myNisa = Math.max(0, myNisa*(1-r)); spouseNisa = Math.max(0, spouseNisa*(1-r)); cash += tk;
          ...
          const na = myNisa + spouseNisa; const tn = Math.min(w, na); const rn = na > 0 ? tn/na : 0;
          myNisa = Math.max(0, myNisa*(1-rn)); spouseNisa = Math.max(0, spouseNisa*(1-rn));
    ...
    rows.push({
      age, year: cy,
      income: Math.round(income), cashFlow: Math.round(cashFlow),
...
export default function LifePlanPro() {
  ...
  const save = () => {
    try { localStorage.setItem("lifeplan_pro_v3", JSON.stringify(p)); setNotice("saved"); setTimeout(() => setNotice(null), 2500); } catch(e) {}
  };
  const share = () => {
    try {
      const url = location.href.split("#")[0] + "#" + btoa(unescape(encodeURIComponent(JSON.stringify(p))));
      navigator.clipboard?.writeText(url);
      setNotice("shared"); setTimeout(() => setNotice(null), 2500);
    } catch(e) {}
  };
  const exportCSV = (data) => {
    const rows = [["西暦","本人歳","収入","収支","生活費","総資産"],
      ...data.map(d => [d.year, d.age, d.income, d.cashFlow, d.livingCost, d.total])];
    const csv = rows.map(r => r.join(",")).join("\n");
    const a = document.createElement("a");
    a.href = "data:text/csv;charset=utf-8," + encodeURIComponent("\uFEFF" + csv);
    a.download = "lifeplan.csv"; a.click();
  };
  ...
  const iLabels = [
    `〜${p.investPeriod1End}`,
    `${p.investPeriod1End+1}〜${p.investPeriod2End}`,
    `${p.investPeriod2End+1}〜`,
  ];
  ...
  const addHouse = () => setP(prev => ({
    ...prev,
    houses: [...(prev.houses || []), { year: 5, cashPrice: 0, loan: 0, loanRate: 1.5, loanYears: 35, investWithdraw: 0, investSource: "nisa" }]
  }));
  ...
  const addSeg = () => setP(prev => {
    ...
    const lastSeg = s[s.length - 2];
    const newEndYear = lastSeg?.endYear ? lastSeg.endYear + 5 : BASE_YEAR + 5;
    s.splice(s.length - 1, 0, { endYear: newEndYear, amount: s[s.length-1]?.amount || 120 });
            ...
            <Card>
              ...
              {p.hasSpouse && (
                <div>
                  ...
                  <Row label="現在のNISA残高"><Num value={p.spouseNisaBalance} onChange={set("spouseNisaBalance")} min={0} max={9999} step={10} unit="万円" width={80} color={C.purple} /></Row>
          ...
          <div>
            <div style={{ background: "linear-gradient(135deg,#065F46,#10B981)", borderRadius: 16, padding: 20, marginBottom: 16, color: "#fff" }}>
              ...
              <div style={{ fontSize: 20, fontWeight: 800, marginTop: 12 }}>
                年間支出合計: <span style={{ color: "#FDE68A" }}>{Math.round(p.food+p.daily+p.utility+p.comm+p.transport+p.leisure+p.insurance+p.clothing+p.medical+p.other).toLocaleString()}万円</span>
              </div>
            </div>
            ...
            <Card color={C.accent + "40"}>
              ...
              {p.housingType === "own" && (
                <div style={{ fontSize: 11, color: C.muted, paddingBottom: 8 }}>持ち家の詳細はSTEP4で設定してください</div>
              )}
            </Card>
            ...
            {p.housingType === "rent" && (
              <Card color={C.green + "40"}>
                ...
                {(p.rentSegments || [{ amount: 120 }]).map((seg, i) => {
                  ...
                  const prevEndYear = i === 0 ? BASE_YEAR - 1 : (segs[i-1].endYear || BASE_YEAR);
                  const label = isLast
                    ? `${prevEndYear + 1}年〜`
                    : `${prevEndYear + 1}〜${seg.endYear}年`;
                  return (
            ...
            {p.childCount > 0 && (
              <Card color={C.yellow + "40"}>
                ...
                <Row label="幼稚園（3〜5歳）"><Num value={p.eduNursery} onChange={set("eduNursery")} min={0} max={200} step={5} unit="万円" width={80} color={C.yellow} /></Row>
          ...
          <div>
            <div style={{ background: "linear-gradient(135deg,#1E3A5F,#3B82F6)", borderRadius: 16, padding: 20, marginBottom: 16, color: "#fff" }}>
              ...
              <div style={{ fontSize: 18, fontWeight: 800, marginTop: 12 }}>
                月間投資: <span style={{ color: "#FDE68A" }}>{Math.round((p.myNisaAnnual1+(p.hasSpouse?p.spouseNisaAnnual1:0)+p.myStockAnnual1+(p.hasSpouse?p.spouseStockAnnual1:0))/12)}万円</span>
              </div>
            </div>
            ...
            <Card color={C.green + "40"}>
              ...
              <P3 vals={[p.myNisaAnnual1, p.myNisaAnnual2, p.myNisaAnnual3]} onChange={(i, v) => setP3("myNisaAnnual", i, v)} max={360} step={12} labels={iLabels} />
            ...
            {p.hasSpouse && (
              <Card color="#34D39940">
                ...
                <P3 vals={[p.spouseNisaAnnual1, p.spouseNisaAnnual2, p.spouseNisaAnnual3]} onChange={(i, v) => setP3("spouseNisaAnnual", i, v)} max={360} step={12} labels={iLabels} />
          ...
          <div>
            ...
            {(p.events || []).map(ev => (
              <div key={ev.id} style={{ background: C.panel, borderRadius: 12, padding: "12px 14px", marginBottom: 8, border: `1px solid ${C.purple}30`, display: "flex", alignItems: "center", gap: 10 }}>
                ...
                <div style={{ flex: 1 }}>
                  <input value={ev.name}
                    onChange={e => setP(prev => ({ ...prev, events: prev.events.map(x => x.id === ev.id ? { ...x, name: e.target.value } : x) }))}
            ...
            <div style={{ background: C.panel, borderRadius: 14, padding: 14, border: `2px dashed ${C.purple}40` }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 8 }}>
                {[{icon:"🚗",name:"車の購入",amt:300},{icon:"🔧",name:"車の買替",amt:200},{icon:"✈️",name:"旅行",amt:50},{icon:"🛋️",name:"家具・家電",amt:100},{icon:"🏥",name:"医療・介護",amt:200},{icon:"🎓",name:"教育費",amt:100},{icon:"💍",name:"冠婚葬祭",amt:100},{icon:"🖥️",name:"PC・機器",amt:30},{icon:"💰",name:"その他",amt:100}].map(cat => (
                  <button key={cat.name}
                    onClick={() => setP(prev => ({ ...prev, events: [...(prev.events || []), { id: Date.now(), name: cat.name, year: 3, amount: cat.amt, icon: cat.icon }] }))}
          ...
          <div>
            ...
            <div style={{ background: C.panel, borderRadius: 16, padding: "14px 10px 10px", marginBottom: 12, border: `1px solid ${C.border}` }}>
              ...
              <ResponsiveContainer width="100%" height={220}>
                <AreaChart data={data} margin={{ top: 14, right: 4, left: -10, bottom: 0 }}>
                  ...
                  {p.hasSpouse && <Area type="monotone" dataKey="spouseNisaAsset" name="配偶者NISA" stroke="#34D399" fill="url(#gn2)" strokeWidth={1.5} dot={false} stackId="a" />}
                  ...
                  {(p.housingType === "own" || (p.houses || []).length > 0) && (
                    <Area type="monotone" dataKey="propVal" name="不動産評価額" stroke="#94A3B8" fill="url(#gp)" strokeWidth={1.5} dot={false} stackId="a" />
                  )}
                </AreaChart>
              </ResponsiveContainer>
            </div>
            ...
            <div style={{ background: "linear-gradient(135deg,#1A2332,#0F2027)", borderRadius: 16, padding: 16, marginBottom: 12 }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8 }}>
                {[["💾", "保存", save, "#7C3AED"], ["🔗", "共有リンク", share, "#0369A1"], ["📋", "CSV出力", () => exportCSV(data), "#166834"]].map(([ic, lb, fn, bg]) => (
