...
const DEFAULTS = {
  ...
  // STEP3: NISA（積立/成長に分離）
  myNisaTsumiAnnual1: 0, myNisaTsumiAnnual2: 0, myNisaTsumiAnnual3: 0,
  myNisaGrowthAnnual1: 0, myNisaGrowthAnnual2: 0, myNisaGrowthAnnual3: 0,
  myNisaStart: BASE_YEAR, myNisaEnd: BASE_YEAR+35,
  myNisaTsumiReturn: 5.0, myNisaGrowthReturn: 6.0,
  spouseNisaTsumiAnnual1: 0, spouseNisaTsumiAnnual2: 0, spouseNisaTsumiAnnual3: 0,
  spouseNisaGrowthAnnual1: 0, spouseNisaGrowthAnnual2: 0, spouseNisaGrowthAnnual3: 0,
  ...
  cashReturn: 0.3,
  investPeriod1End: BASE_YEAR+4, investPeriod2End: BASE_YEAR+9,
...
function simulate(p) {
  ...
  let spouseNisaTsumi = p.spouseNisaTsumiBalance||0, spouseNisaGrowth = p.spouseNisaGrowthBalance||0;
  ...
  for (let age = p.myAge; age <= p.lifeExpectancy; age++) {
    ...
    const spouseRaw = !p.hasSpouse ? 0
      : p.useIncomeCoef ? applyCoef(p.spouseTakeHome, p.spouseAge, p.spouseAge+yr)
      : p.spouseTakeHome * Math.pow(1+p.spouseIncomeGrowth/100, Math.min(yr, Math.max(0,55-p.spouseAge)));
    const spouseInc = !p.hasSpouse ? 0 : spouseRetired ? p.spousePension
      : inAdj ? spouseRaw*(p.spouseAdjRatio/100) : spouseRaw;
    ...
    const propTax = isOwned&&p.propertyTaxEnabled&&basePrice>0
    ...
    // 教育費
    const edu = ca0 => { const ca=ca0+yr; if(ca>=0&&ca<3) return p.eduNursery; if(ca>=3&&ca<6) return p.eduNursery*0.5; if(ca>=6&&ca<12) return p.eduElementary; if(ca>=12&&ca<15) return p.eduJunior; if(ca>=15&&ca<18) return p.eduHigh; if(ca>=18&&ca<22) return p.eduCollege; return 0; };
    const eduCost = (p.child1Age>=0?edu(p.child1Age):0)+(p.child2Age>=0?edu(p.child2Age):0);

    // 投資（NISA分割・期間別）
    const iP = cy<=p.investPeriod1End ? 1 : cy<=p.investPeriod2End ? 2 : 3;
    const g = k => p[k+iP]||0;
    const inNisa = !bothRetired && cy>=p.myNisaStart && cy<=p.myNisaEnd;
    const myNisaTsumiC = inNisa ? g("myNisaTsumiAnnual") : 0;
    const myNisaGrowthC = inNisa ? g("myNisaGrowthAnnual") : 0;
    const inSpNisa = p.hasSpouse && !bothRetired && cy>=p.spouseNisaStart && cy<=p.spouseNisaEnd;
    const spouseNisaTsumiC = inSpNisa ? g("spouseNisaTsumiAnnual") : 0;
    const spouseNisaGrowthC = inSpNisa ? g("spouseNisaGrowthAnnual") : 0;
    const myStockC = !bothRetired && cy>=p.myStockStart && cy<=p.myStockEnd ? g("myStockAnnual") : 0;
    const spouseStockC = p.hasSpouse&&!bothRetired&&cy>=p.spouseStockStart&&cy<=p.spouseStockEnd ? g("spouseStockAnnual") : 0;
    const investTotal = myNisaTsumiC+myNisaGrowthC+spouseNisaTsumiC+spouseNisaGrowthC+myStockC+spouseStockC;

    // 車維持費（毎年）
    const carMaintCost = p.hasCarMaint ? (p.carGasoline||6)+(p.carTax||4)+(p.carInsurance||6) : 0;
    // 定期出費（車検・タイヤ等）
    const recurringCost = (p.recurringEvents||[])
      .filter(ev => ev.startYear<=yr && (yr-ev.startYear)%ev.intervalYears===0)
      .reduce((s,ev)=>s+ev.amount,0);
    // 一時出費
    const eventCost = (p.events||[]).filter(ev=>ev.year===yr).reduce((s,ev)=>s+ev.amount,0);

    const totalCost = annualLiving+rentY+loanCost+propTax+eduCost+p.insurance+investTotal+carMaintCost+recurringCost;
    ...
    spouseNisaTsumi = spouseNisaTsumi*(1+(p.spouseNisaTsumiReturn||5)/100)+spouseNisaTsumiC;
    spouseNisaGrowth = spouseNisaGrowth*(1+(p.spouseNisaGrowthReturn||6)/100)+spouseNisaGrowthC;
    myStock = myStock*(1+(p.myStockReturn||6)/100)+myStockC;
    spouseStock = spouseStock*(1+(p.spouseStockReturn||6)/100)+spouseStockC;
    ...
    cash = cash*(1+(p.cashReturn||0.3)/100)+cashFlow+myBonus+spouseBonus-cashBuy-eventCost-k1c-k2c;

    houses.forEach(h => {
      if (h.investWithdraw>0 && yr===h.year) {
        const w=h.investWithdraw;
        const allNisa = myNisaTsumi+myNisaGrowth+spouseNisaTsumi+spouseNisaGrowth;
        if (h.investSource==="nisa") {
          const tk=Math.min(w,allNisa); const r=allNisa>0?tk/allNisa:0;
          myNisaTsumi=Math.max(0,myNisaTsumi*(1-r)); myNisaGrowth=Math.max(0,myNisaGrowth*(1-r));
          spouseNisaTsumi=Math.max(0,spouseNisaTsumi*(1-r)); spouseNisaGrowth=Math.max(0,spouseNisaGrowth*(1-r));
          cash+=tk;
        } else if (h.investSource==="stock") {
          const av=myStock+spouseStock; const tk=Math.min(w,av); const r=av>0?tk/av:0;
          myStock=Math.max(0,myStock*(1-r)); spouseStock=Math.max(0,spouseStock*(1-r)); cash+=tk;
        } else {
          const tn=Math.min(w,allNisa); const rn=allNisa>0?tn/allNisa:0;
          myNisaTsumi=Math.max(0,myNisaTsumi*(1-rn)); myNisaGrowth=Math.max(0,myNisaGrowth*(1-rn));
          spouseNisaTsumi=Math.max(0,spouseNisaTsumi*(1-rn)); spouseNisaGrowth=Math.max(0,spouseNisaGrowth*(1-rn));
          const sa=myStock+spouseStock; const ts=Math.min(w-tn,sa); const rs=sa>0?ts/sa:0;
          myStock=Math.max(0,myStock*(1-rs)); spouseStock=Math.max(0,spouseStock*(1-rs)); cash+=tn+ts;
        }
      }
    });
    ...
    rows.push({
      ...
      carCost:Math.round(carMaintCost+recurringCost),
      cashAsset:Math.round(cash),
      myNisaTsumiAsset:Math.round(myNisaTsumi), myNisaGrowthAsset:Math.round(myNisaGrowth),
      spouseNisaTsumiAsset:Math.round(spouseNisaTsumi), spouseNisaGrowthAsset:Math.round(spouseNisaGrowth),
      myStockAsset:Math.round(myStock), spouseStockAsset:Math.round(spouseStock),
      kid1NisaAsset:Math.round(kid1), kid2NisaAsset:Math.round(kid2),
      propVal:Math.round(propVal),
      total:Math.round(cash+myNisaTsumi+myNisaGrowth+spouseNisaTsumi+spouseNisaGrowth+myStock+spouseStock+kid1+kid2+propVal),
    });
  }
  return rows;
}
...
const Num = ({ value, onChange, min=0, max=9999, step=1, unit="", color=C.accent, width=72 }) => {
  ...
  const bs = side => ({
    width:32, height:32,
    borderRadius: side==="l" ? "8px 0 0 8px" : "0 8px 8px 0",
    border:`2px solid ${color}`,
    borderLeft: side==="r" ? "none" : undefined,
...
const Toggle = ({value, onChange, color=C.accent}) => (
  <button onClick={()=>onChange(!value)}
    style={{width:48,height:26,borderRadius:13,border:"none",cursor:"pointer",background:value?color:"#CBD5E1",position:"relative",flexShrink:0}}>
...
const P3 = ({vals, onChange, max, step=1, unit="万", labels}) => (
  <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8,marginTop:8,marginBottom:4}}>
    {labels.map((lbl,i) => (
      <div key={i}>
        <div style={{fontSize:10,color:C.muted,marginBottom:4,textAlign:"center",lineHeight:1.3}}>{lbl}</div>
        <Num value={vals[i]} onChange={v=>onChange(i,v)} min={0} max={max} step={step} unit={unit} width={52}/>
      </div>
    ))}
  </div>
);
...
const AiAdvice = ({p, data}) => {
  ...
  const getAdvice = async () => {
    ...
    try {
      ...
      const totalInvestAnnual = (p.myNisaTsumiAnnual1||0)+(p.myNisaGrowthAnnual1||0)
        +(p.hasSpouse?(p.spouseNisaTsumiAnnual1||0)+(p.spouseNisaGrowthAnnual1||0):0)
        +(p.myStockAnnual1||0)+(p.hasSpouse?p.spouseStockAnnual1||0:0);
      const totalIncome = (p.myTakeHome||0)+(p.hasSpouse?p.spouseTakeHome||0:0);
      const investRatio = totalIncome>0 ? Math.round(totalInvestAnnual/totalIncome*100) : 0;
      const summary = {
        年齢: p.myAge, 家族構成: p.hasSpouse?"既婚":"独身", 子供: p.childCount+"人",
        手取り年収合計: totalIncome+"万円",
        年間投資額: totalInvestAnnual+"万円", 投資比率: investRatio+"%",
        退職時資産: Math.round(retireRow.total)+"万円",
        最終資産: Math.round(finalRow.total)+"万円",
        資産枯渇: crossZero ? crossZero.age+"歳で枯渇" : "枯渇なし",
        NISA積立: (p.myNisaTsumiAnnual1||0)+"万円/年",
        NISA成長: (p.myNisaGrowthAnnual1||0)+"万円/年",
        証券: (p.myStockAnnual1||0)+"万円/年",
        住まい: p.housingType==="rent"?"賃貸":"持ち家",
        車: p.hasCarMaint?"あり":"なし",
      };
      const res = await fetch("https://api.anthropic.com/v1/messages", {
        ...
        body: JSON.stringify({
          ...
          max_tokens:1000,
          system:`あなたはFP（ファイナンシャルプランナー）のAIアシスタントです。ユーザーのライフプランデータを分析し、投資バランスと資産形成についての具体的なアドバイスを日本語で提供してください。
アドバイスは以下の構成でMarkdownなしの箇条書き（絵文字付き）で返してください：
1. 総評（1〜2文）
2. 投資バランスの評価（NISA積立・成長枠・証券口座のバランス）
3. 具体的な改善提案（2〜3点）
4. リスク・注意点
回答は500文字以内で簡潔に。`,
          messages:[{role:"user",content:`以下のライフプランデータについてアドバイスをください：\n${JSON.stringify(summary,null,2)}`}]
        })
      });
      const d = await res.json();
      const text = d.content?.find(c=>c.type==="text")?.text||"アドバイスを取得できませんでした。";
      setAdvice(text);
    } catch(e) {
      setAdvice("⚠️ アドバイスの取得に失敗しました。しばらくしてから再試行してください。");
    }
    setLoading(false);
  };

  return (
    <div style={{background:"linear-gradient(135deg,#1A1A2E,#16213E)",borderRadius:16,padding:16,marginBottom:12,border:`1px solid ${C.purple}50`}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:advice||loading?12:0}}>
        <div>
          <div style={{fontSize:14,fontWeight:800,color:"#E2E8F0"}}>🤖 AIアドバイザー</div>
          <div style={{fontSize:11,color:"#64748B",marginTop:2}}>投資バランスと資産形成についてアドバイスをもらいます</div>
        </div>
        <button onClick={getAdvice} disabled={loading}
          style={{padding:"10px 16px",borderRadius:10,border:"none",cursor:loading?"not-allowed":"pointer",background:loading?"#374151":`linear-gradient(135deg,${C.purple},#6366F1)`,color:"#fff",fontWeight:700,fontSize:12,flexShrink:0,opacity:loading?0.7:1}}>
          {loading?"分析中…":"✨ 分析する"}
        </button>
      </div>
      {loading && (
        <div style={{background:"#1E293B",borderRadius:10,padding:"14px 16px"}}>
          <div style={{display:"flex",alignItems:"center",gap:10}}>
            <div style={{width:16,height:16,borderRadius:"50%",border:`2px solid ${C.purple}`,borderTopColor:"transparent",animation:"spin 0.8s linear infinite"}}/>
            <span style={{fontSize:12,color:"#94A3B8"}}>ライフプランを分析しています…</span>
          </div>
        </div>
      )}
      {advice && !loading && (
        <div style={{background:"#0F172A",borderRadius:10,padding:"14px 16px"}}>
          {advice.split("\n").filter(l=>l.trim()).map((line,i) => (
            <div key={i} style={{fontSize:12,color:"#CBD5E1",lineHeight:1.7,marginBottom:4}}>{line}</div>
          ))}
        </div>
      )}
...
export default function LifePlanPro() {
  const [p, setP] = useState(() => {
    try {
      ...
      const saved = localStorage.getItem("lifeplan_pro_v4");
  ...
  const save = () => {
    try { localStorage.setItem("lifeplan_pro_v4",JSON.stringify(p)); setNotice("saved"); setTimeout(()=>setNotice(null),2500); } catch(e){}
  };
  const share = () => {
    try {
      const url = location.href.split("#")[0]+"#"+btoa(unescape(encodeURIComponent(JSON.stringify(p))));
      navigator.clipboard?.writeText(url); setNotice("shared"); setTimeout(()=>setNotice(null),2500);
    } catch(e){}
  };
  const exportCSV = () => {
    const rows = [["西暦","本人歳","収入","収支","生活費","車費","総資産"],
      ...data.map(d=>[d.year,d.age,d.income,d.cashFlow,d.livingCost,d.carCost,d.total])];
  ...
  const retireRow = data.find(d=>d.age===p.myRetireAge);
  const peakTotal = Math.max(0,...data.map(d=>d.total));
  const crossZero = data.find(d=>d.total<0);
  const finalRow = data[data.length-1];
  const minusYears = data.filter(d=>d.cashFlow<0 && d.age<p.myRetireAge);

  const totalMonthlyInvest = Math.round(((p.myNisaTsumiAnnual1||0)+(p.myNisaGrowthAnnual1||0)
    +(p.hasSpouse?(p.spouseNisaTsumiAnnual1||0)+(p.spouseNisaGrowthAnnual1||0):0)
    +(p.myStockAnnual1||0)+(p.hasSpouse?p.spouseStockAnnual1||0:0))/12);

  const iLabels = [`〜${p.investPeriod1End}`,`${p.investPeriod1End+1}〜${p.investPeriod2End}`,`${p.investPeriod2End+1}〜`];
  ...
  const addSeg = () => setP(prev=>{
    const s=[...(prev.rentSegments||[{amount:120}])];
        ...
        <div style={{maxWidth:640,margin:"0 auto"}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
            <div>
              ...
              <div style={{fontSize:10,color:"#64748B"}}>v4.0</div>
            </div>
          ...
          {notice && (
            <div style={{background:notice==="saved"?"#065F46":"#1E3A5F",borderRadius:8,padding:"6px 12px",fontSize:12,color:"#fff",fontWeight:700,textAlign:"center",marginBottom:8}}>
              {notice==="saved"?"✅ 保存しました":"✅ リンクをコピーしました"}
            </div>
          )}
          <div style={{display:"flex"}}>
            {STEPS.map(s=>(
        ...
        {step===1 && (
          <div>
            ...
            <Card color={C.accent+"40"}>
              ...
              <Row label="収入変動係数（FP標準値）を使う" sub="年齢帯別に自動で収入成長を反映">
              ...
              <Row label="NISA積立投資枠 残高"><Num value={p.myNisaTsumiBalance} onChange={set("myNisaTsumiBalance")} min={0} max={9999} step={10} unit="万円" width={80}/></Row>
              <Row label="NISA成長投資枠 残高"><Num value={p.myNisaGrowthBalance} onChange={set("myNisaGrowthBalance")} min={0} max={9999} step={10} unit="万円" width={80}/></Row>
            </Card>
            <Card>
              ...
              {p.hasSpouse && (
                <div>
                  ...
                  {!p.useIncomeCoef && <Row label="年上昇率"><Num value={p.spouseIncomeGrowth} onChange={set("spouseIncomeGrowth")} min={0} max={5} step={0.1} unit="%" color={C.purple}/></Row>}
                  ...
                  <Row label="NISA積立投資枠 残高"><Num value={p.spouseNisaTsumiBalance} onChange={set("spouseNisaTsumiBalance")} min={0} max={9999} step={10} unit="万円" width={80} color={C.purple}/></Row>
                  <Row label="NISA成長投資枠 残高"><Num value={p.spouseNisaGrowthBalance} onChange={set("spouseNisaGrowthBalance")} min={0} max={9999} step={10} unit="万円" width={80} color={C.purple}/></Row>
            ...
            <Card>
              ...
              <Row label="子どもの人数">
                <div style={{display:"flex",gap:8}}>
                  {[0,1,2].map(n=>(
                    <Chip key={n} label={n===0?"なし":`${n}人`} selected={p.childCount===n}
                      onClick={()=>setP(prev=>({...prev,childCount:n,child1Age:n>=1?(prev.child1Age<0?0:prev.child1Age):-1,child2Age:n>=2?(prev.child2Age<0?0:prev.child2Age):-1}))}/>
                  ))}
                </div>
              </Row>
              {p.childCount>=1 && <Row label="第1子の年齢"><Num value={p.child1Age} onChange={set("child1Age")} min={0} max={18} unit="歳" color={C.yellow}/></Row>}
              {p.childCount>=2 && <Row label="第2子の年齢"><Num value={p.child2Age} onChange={set("child2Age")} min={0} max={18} unit="歳" color={C.yellow}/></Row>}
          ...
          <div>
              ...
              <div style={{display:"flex",gap:10,paddingTop:8,paddingBottom:8}}>
                {[["rent","🏢 賃貸"],["own","🏡 持ち家（既に保有）"]].map(([v,l])=>(
                  <button key={v} onClick={()=>set("housingType")(v)}
                    style={{flex:1,padding:"12px 8px",borderRadius:12,border:`2px solid ${p.housingType===v?C.accent:C.border}`,background:p.housingType===v?C.accent:"#fff",color:p.housingType===v?"#fff":C.muted,fontSize:12,fontWeight:700,cursor:"pointer"}}>
                    {l}
                  </button>
                ))}
              </div>
            ...
            {p.housingType==="rent" && (
              <Card color={C.green+"40"}>
                ...
                {(p.rentSegments||[{amount:120}]).map((seg,i)=>{
                  ...
                  const label = isLast ? `${prevEnd+1}年〜` : `${prevEnd+1}〜${seg.endYear}年`;
                  return (
                ...
                <button onClick={addSeg} style={{width:"100%",padding:10,borderRadius:10,border:`2px dashed ${C.green}`,background:"#F0FDF4",color:C.green,fontWeight:700,fontSize:13,cursor:"pointer"}}>
                  ＋ 期間を追加（家賃変動）
                </button>
              </Card>
            )}
              ...
              {p.hasCarMaint && (
                <div>
                  ...
                  <div style={{background:"#FFF7ED",borderRadius:8,padding:"8px 10px",marginTop:6,fontSize:11,color:"#92400E"}}>
                    毎年の車維持費合計: 約{(p.carGasoline+p.carTax+p.carInsurance).toLocaleString()}万円/年
                  </div>
                  <SH title="定期出費（車検・タイヤ等）" icon="🔧" color={C.muted}/>
                  <div style={{fontSize:11,color:C.muted,marginBottom:8}}>数年おきに発生する出費を設定できます</div>
                  {(p.recurringEvents||[]).map(ev=>(
                    <div key={ev.id} style={{background:"#FFF7ED",borderRadius:10,padding:"10px 12px",marginBottom:8,border:`1px solid ${C.orange}30`}}>
                      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:6}}>
                        <input value={ev.name} onChange={e=>updateRecurring(ev.id,"name",e.target.value)}
                          style={{fontSize:12,fontWeight:700,border:"none",borderBottom:`1.5px solid ${C.orange}`,background:"transparent",outline:"none",color:C.dark,width:"60%"}}/>
                      ...
                      <div style={{display:"flex",gap:10,flexWrap:"wrap",alignItems:"center"}}>
                        <div>
                          <div style={{fontSize:10,color:C.muted,marginBottom:2}}>金額</div>
                          <Num value={ev.amount} onChange={v=>updateRecurring(ev.id,"amount",v)} min={1} max={200} step={1} unit="万円" color={C.orange} width={56}/>
                        </div>
                        <div>
                          <div style={{fontSize:10,color:C.muted,marginBottom:2}}>周期</div>
                          <Num value={ev.intervalYears} onChange={v=>updateRecurring(ev.id,"intervalYears",v)} min={1} max={10} step={1} unit="年おき" color={C.orange} width={40}/>
                        </div>
                        <div>
                          <div style={{fontSize:10,color:C.muted,marginBottom:2}}>初回</div>
                          <Num value={ev.startYear} onChange={v=>updateRecurring(ev.id,"startYear",v)} min={0} max={30} step={1} unit="年後" color={C.orange} width={40}/>
                        </div>
                      </div>
                    </div>
                  ))}
                  <button onClick={addRecurring} style={{width:"100%",padding:"8px",borderRadius:10,border:`2px dashed ${C.orange}`,background:"#FFF7ED",color:C.orange,fontWeight:700,fontSize:12,cursor:"pointer"}}>
                    ＋ 定期出費を追加
                  </button>
                </div>
              )}
            ...
            {p.childCount>0 && (
              <Card color={C.yellow+"40"}>
                ...
                <Row label="保育園（0〜2歳）"><Num value={p.eduNursery} onChange={set("eduNursery")} min={0} max={200} step={5} unit="万円" width={80} color={C.yellow}/></Row>
        ...
        {step===3 && (
          <div>
            <div style={{background:"linear-gradient(135deg,#1E3A5F,#3B82F6)",borderRadius:16,padding:20,marginBottom:16,color:"#fff"}}>
              ...
              <div style={{fontSize:12,opacity:0.8}}>NISA（積立・成長）＋証券口座を設定します</div>
              <div style={{fontSize:18,fontWeight:800,marginTop:12}}>
                月間投資: <span style={{color:"#FDE68A"}}>{totalMonthlyInvest}万円</span>
              </div>
            </div>
            ...
            <div style={{background:"#EFF6FF",borderRadius:12,padding:"12px 14px",marginBottom:12}}>
              <div style={{fontSize:12,fontWeight:700,color:C.accent,marginBottom:6}}>📅 投資期間の区切り年</div>
              <div style={{display:"flex",gap:10,alignItems:"center"}}>
                <div style={{flex:1}}>
                  <div style={{fontSize:10,color:C.muted,marginBottom:3}}>期間1の終わり</div>
                  <Num value={p.investPeriod1End} onChange={v=>setP(prev=>({...prev,investPeriod1End:Math.min(v,prev.investPeriod2End-1)}))} min={BASE_YEAR} max={2060} unit="年まで" color={C.accent} width={64}/>
                </div>
                <div style={{fontSize:18,color:C.muted}}>→</div>
                <div style={{flex:1}}>
                  <div style={{fontSize:10,color:C.muted,marginBottom:3}}>期間2の終わり</div>
                  <Num value={p.investPeriod2End} onChange={v=>setP(prev=>({...prev,investPeriod2End:Math.max(v,prev.investPeriod1End+1)}))} min={BASE_YEAR+1} max={2065} unit="年まで" color={C.accent} width={64}/>
                </div>
              </div>
              <div style={{fontSize:10,color:C.muted,marginTop:6}}>期間1:〜{p.investPeriod1End}年　期間2:{p.investPeriod1End+1}〜{p.investPeriod2End}年　期間3:{p.investPeriod2End+1}年〜</div>
            </div>

            {/* 本人NISA（積立・成長に分割） */}
            <Card color={C.green+"40"}>
              <SH title="本人 NISA" icon="📗" color={C.green}/>
              <div style={{background:"#F0FDF4",borderRadius:8,padding:"8px 10px",marginBottom:8,fontSize:11,color:C.green}}>
                💡 年間上限360万円（積立120万＋成長240万）
              </div>
              <div style={{fontSize:12,fontWeight:700,color:C.green,margin:"8px 0 4px"}}>積立投資枠（上限120万/年・インデックスファンド等）</div>
              <P3 vals={[p.myNisaTsumiAnnual1,p.myNisaTsumiAnnual2,p.myNisaTsumiAnnual3]} onChange={(i,v)=>setP3("myNisaTsumiAnnual",i,v)} max={120} step={6} labels={iLabels}/>
              <Row label="積立枠 想定利回り"><Num value={p.myNisaTsumiReturn} onChange={set("myNisaTsumiReturn")} min={0} max={12} step={0.1} unit="%" color={C.green}/></Row>
              <div style={{fontSize:12,fontWeight:700,color:"#059669",margin:"12px 0 4px"}}>成長投資枠（上限240万/年・個別株・ETF等）</div>
              <P3 vals={[p.myNisaGrowthAnnual1,p.myNisaGrowthAnnual2,p.myNisaGrowthAnnual3]} onChange={(i,v)=>setP3("myNisaGrowthAnnual",i,v)} max={240} step={12} labels={iLabels}/>
              <Row label="成長枠 想定利回り"><Num value={p.myNisaGrowthReturn} onChange={set("myNisaGrowthReturn")} min={0} max={15} step={0.1} unit="%" color="#059669"/></Row>
              ...
              {(p.myNisaTsumiAnnual1+p.myNisaGrowthAnnual1)>360 && (
                <div style={{background:"rgba(239,68,68,0.1)",borderRadius:8,padding:"6px 10px",marginTop:6,fontSize:11,color:C.red}}>⚠️ 年間上限360万円を超えています</div>
              )}
            </Card>
            ...
            {p.hasSpouse && (
              <Card color="#34D39940">
                ...
                <div style={{background:"#F0FDF4",borderRadius:8,padding:"8px 10px",marginBottom:8,fontSize:11,color:"#34D399"}}>
                  💡 年間上限360万円（積立120万＋成長240万）
                </div>
                <div style={{fontSize:12,fontWeight:700,color:"#34D399",margin:"8px 0 4px"}}>積立投資枠（上限120万/年）</div>
                <P3 vals={[p.spouseNisaTsumiAnnual1,p.spouseNisaTsumiAnnual2,p.spouseNisaTsumiAnnual3]} onChange={(i,v)=>setP3("spouseNisaTsumiAnnual",i,v)} max={120} step={6} labels={iLabels}/>
                <Row label="積立枠 想定利回り"><Num value={p.spouseNisaTsumiReturn} onChange={set("spouseNisaTsumiReturn")} min={0} max={12} step={0.1} unit="%" color="#34D399"/></Row>
                <div style={{fontSize:12,fontWeight:700,color:"#059669",margin:"12px 0 4px"}}>成長投資枠（上限240万/年）</div>
                <P3 vals={[p.spouseNisaGrowthAnnual1,p.spouseNisaGrowthAnnual2,p.spouseNisaGrowthAnnual3]} onChange={(i,v)=>setP3("spouseNisaGrowthAnnual",i,v)} max={240} step={12} labels={iLabels}/>
                <Row label="成長枠 想定利回り"><Num value={p.spouseNisaGrowthReturn} onChange={set("spouseNisaGrowthReturn")} min={0} max={15} step={0.1} unit="%" color="#059669"/></Row>
            ...
            <Card color={C.accent+"40"}>
              ...
              <P3 vals={[p.myStockAnnual1,p.myStockAnnual2,p.myStockAnnual3]} onChange={(i,v)=>setP3("myStockAnnual",i,v)} max={600} step={12} labels={iLabels}/>
            ...
            {p.hasSpouse && (
              <Card color="#93C5FD40">
                <SH title="配偶者 証券口座（特定口座）" icon="📈" color="#93C5FD"/>
                <P3 vals={[p.spouseStockAnnual1,p.spouseStockAnnual2,p.spouseStockAnnual3]} onChange={(i,v)=>setP3("spouseStockAnnual",i,v)} max={600} step={12} labels={iLabels}/>
          ...
          <div>
            ...
            {p.housingType==="own" && (
              <Card color={C.yellow+"40"}>
                ...
                {p.propertyTaxEnabled && <Row label="実効税率" sub="一般的に購入価格の0.1〜0.2%程度"><Num value={p.propertyTaxRate||0.14} onChange={set("propertyTaxRate")} min={0.05} max={0.5} step={0.01} unit="%" color={C.orange}/></Row>}
              </Card>
            )}
            <Card>
              ...
              <div style={{display:"flex",gap:8,paddingTop:8}}>
                {[0,1,2,3].map(n=>(
                  <button key={n} onClick={()=>{
                    const cur=p.houses||[];
                    const next=n>cur.length ? [...cur,...Array(n-cur.length).fill(null).map((_,i)=>({year:5+(i+cur.length)*10,cashPrice:0,loan:0,loanRate:1.5,loanYears:35,investWithdraw:0,investSource:"nisa"}))] : cur.slice(0,n);
              ...
              return (
                <Card key={i} color={col+"40"}>
                  ...
                  {(h.cashPrice>0||h.loan>0) && (
                    <div>
                      <SH title="固定資産税・評価額" icon="📋" color={C.orange}/>
                      <Row label="固定資産税を計算する"><Toggle value={p.propertyTaxEnabled} onChange={set("propertyTaxEnabled")} color={C.orange}/></Row>
          ...
          <div>
            ...
            <div style={{background:C.panel,borderRadius:16,padding:"14px 10px 10px",marginBottom:12,border:`1px solid ${C.border}`}}>
              ...
              <ResponsiveContainer width="100%" height={220}>
                <AreaChart data={data} margin={{top:14,right:4,left:-10,bottom:0}}>
                  ...
                  <Tooltip content={({active,payload,label})=>{
                    ...
                    return (
                      <div style={{background:C.dark,borderRadius:10,padding:"10px 14px",fontSize:12}}>
                        ...
                        {payload.filter(x=>x.value>0).map((x,i)=>(
                          <div key={i} style={{color:x.color,marginBottom:2}}>{x.name}: <strong>{Math.round(x.value).toLocaleString()}万円</strong></div>
                        ))}
                      </div>
                    );
                  }}/>
                  ...
                  {p.hasSpouse && <Area type="monotone" dataKey="spouseNisaTsumiAsset" name="配偶者NISA積立" stroke="#34D399" fill="url(#gnt2)" strokeWidth={1.5} dot={false} stackId="a"/>}
                  {p.hasSpouse && <Area type="monotone" dataKey="spouseNisaGrowthAsset" name="配偶者NISA成長" stroke="#047857" fill="url(#gng2)" strokeWidth={1.5} dot={false} stackId="a"/>}
                  <Area type="monotone" dataKey="myStockAsset" name="本人証券" stroke={C.accent} fill="url(#gs)" strokeWidth={1.5} dot={false} stackId="a"/>
                  {p.hasSpouse && <Area type="monotone" dataKey="spouseStockAsset" name="配偶者証券" stroke="#93C5FD" fill="url(#gs2)" strokeWidth={1.5} dot={false} stackId="a"/>}
                ...
                <LineChart data={data} margin={{top:14,right:4,left:-10,bottom:0}}>
                  ...
                  <Tooltip content={({active,payload,label})=>{
                    ...
                    return (
                      <div style={{background:C.dark,borderRadius:10,padding:"10px 14px",fontSize:12}}>
                        <div style={{color:"#93C5FD",fontWeight:700,marginBottom:6}}>{label}歳（{BASE_YEAR+label-p.myAge}年）</div>
                        {payload.map((x,i)=><div key={i} style={{color:x.color,marginBottom:2}}>{x.name}: <strong>{Math.round(x.value).toLocaleString()}万円</strong></div>)}
                      </div>
                    );
                  }}/>
                  ...
                  <Line type="monotone" dataKey="income" name="収入" stroke={C.green} dot={false} strokeWidth={2.5}/>
                  <Line type="monotone" dataKey="livingCost" name="生活費" stroke={C.red} dot={false} strokeWidth={2}/>
                  <Line type="monotone" dataKey="cashFlow" name="収支" stroke={C.orange} dot={false} strokeWidth={2} strokeDasharray="6 2"/>
                </LineChart>
              </ResponsiveContainer>
            ...
            <div style={{background:"linear-gradient(135deg,#1A2332,#0F2027)",borderRadius:16,padding:16,marginBottom:12}}>
              ...
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8}}>
                {[["💾","保存",save,"#7C3AED"],["🔗","共有リンク",share,"#0369A1"],["📋","CSV出力",exportCSV,"#166834"]].map(([ic,lb,fn,bg])=>(
                  <button key={lb} onClick={fn}
                    style={{padding:"12px 4px",borderRadius:10,border:"none",cursor:"pointer",background:bg,color:"#fff",fontWeight:700,fontSize:11,display:"flex",flexDirection:"column",alignItems:"center",gap:3}}>
        ...
        <div style={{display:"flex",gap:10,marginTop:8}}>
          {step>1 && (
            <button onClick={()=>setStep(s=>s-1)}
              style={{flex:1,padding:14,borderRadius:12,border:`2px solid ${C.border}`,background:"#fff",color:C.text,fontSize:15,fontWeight:700,cursor:"pointer"}}>
              ← 戻る
            </button>
          )}
          {step<6 && (
            <button onClick={()=>setStep(s=>s+1)}
              style={{flex:2,padding:14,borderRadius:12,border:"none",background:`linear-gradient(135deg,${C.accent},#2563EB)`,color:"#fff",fontSize:15,fontWeight:700,cursor:"pointer"}}>
              次へ →
            </button>
          )}
          {step===6 && (
            <button onClick={()=>setStep(1)}
              style={{flex:1,padding:14,borderRadius:12,border:`2px solid ${C.accent}`,background:"#fff",color:C.accent,fontSize:14,fontWeight:700,cursor:"pointer"}}>
              ✏️ 設定を編集
            </button>
          )}
        </div>
